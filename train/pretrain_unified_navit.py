...
class DataArguments:
    ...
    gen_aug: bool = field(
        default=False,
        metadata={"help": "Enable generation-side medical image augmentation for selected dataset groups."}
    )
    gen_aug_prob: float = field(
        default=0.5,
        metadata={"help": "Probability of applying generation-side augmentation to each image."}
    )
    gen_aug_strength: float = field(
        default=0.4,
        metadata={"help": "Strength of generation-side augmentation, typically between 0 and 1."}
    )
    gen_aug_ops: str = field(
        default="intensity,gamma,noise,blur,lowres,bias",
        metadata={"help": "Comma-separated generation augmentation ops: intensity,gamma,noise,blur,lowres,bias."}
    )
    gen_aug_groups: str = field(
        default="t2i_pretrain,unified_edit",
        metadata={"help": "Comma-separated dataset groups that should receive generation-side augmentation."}
    )
    gen_spatial_shift: bool = field(
        default=False,
        metadata={"help": "Enable paired generation-side spatial shift augmentation."}
    )
    gen_spatial_shift_prob: float = field(
        default=0.5,
        metadata={"help": "Probability of applying paired spatial shift to a generation sample."}
    )
    gen_spatial_shift_max_ratio: float = field(
        default=0.1,
        metadata={"help": "Maximum x/y translation ratio for paired spatial shift, e.g. 0.1 means up to 10% of width/height."}
    )
    gen_spatial_shift_groups: str = field(
        default="t2i_pretrain,unified_edit",
        metadata={"help": "Comma-separated dataset groups that should receive spatial shift augmentation."}
    )
...
class TrainingArguments:
    ...
    save_every: int = field(
        default=200,
        metadata={"help": "(unused in best-checkpoint mode) keep for compatibility."}
    )
...
def _inject_generation_augmentation(dataset_meta, data_args, logger):
    dataset_meta = deepcopy(dataset_meta)
    target_groups = set(_parse_csv_arg(data_args.gen_aug_groups if data_args.gen_aug else ""))
    target_groups.update(_parse_csv_arg(data_args.gen_spatial_shift_groups if data_args.gen_spatial_shift else ""))
    augmentation_ops = _parse_csv_arg(data_args.gen_aug_ops)
    patched_groups = []
    skipped_groups = []

    for group_name in sorted(target_groups):
        group_cfg = dataset_meta.get(group_name)
        if group_cfg is None:
            skipped_groups.append(f"{group_name} (missing group)")
            continue
        if 'image_transform_args' not in group_cfg:
            skipped_groups.append(f"{group_name} (missing image_transform_args)")
            continue

        image_transform_args = deepcopy(group_cfg['image_transform_args'])
        if data_args.gen_aug:
            image_transform_args.update(
                enable_augmentation=True,
                augmentation_prob=data_args.gen_aug_prob,
                augmentation_ops=augmentation_ops,
                augmentation_strength=data_args.gen_aug_strength,
            )
        if data_args.gen_spatial_shift:
            image_transform_args.update(
                enable_spatial_shift=True,
                spatial_shift_prob=data_args.gen_spatial_shift_prob,
                spatial_shift_max_ratio=data_args.gen_spatial_shift_max_ratio,
            )
        group_cfg['image_transform_args'] = image_transform_args
        patched_groups.append(group_name)

    if patched_groups:
        logger.info(
            "Enabled generation augmentation for groups=%s ops=%s prob=%.3f strength=%.3f spatial_shift=%s spatial_prob=%.3f spatial_max_ratio=%.3f",
            patched_groups,
            augmentation_ops,
            data_args.gen_aug_prob,
            data_args.gen_aug_strength,
            data_args.gen_spatial_shift,
            data_args.gen_spatial_shift_prob,
            data_args.gen_spatial_shift_max_ratio,
        )
    if skipped_groups:
        logger.warning("Skipped generation augmentation for groups=%s", skipped_groups)
...
def main():
    ...
    # ===== 0) rank / device 绑定：在 init_process_group 之前完成 =====
    local_rank = int(os.environ.get("LOCAL_RANK", 0))
    rank = int(os.environ.get("RANK", 0))
    world_size = int(os.environ.get("WORLD_SIZE", 1))

    assert torch.cuda.is_available(), "CUDA is not available"
    torch.cuda.set_device(local_rank)  # 关键：先绑定本进程 GPU
    ...
    def _to_cpu_only(x):
        ...
        if isinstance(x, dict):
            return {k: _to_cpu_only(v) for k, v in x.items()}
        if isinstance(x, (list, tuple)):
            return [_to_cpu_only(v) for v in x]
        return x

    # 兼容不同 torch 版本：优先传 device_id=torch.device，失败则回退
    try:
        dist.init_process_group(
            ...
            device_id=device,  # 新版支持；老版会 TypeError
        )
    except TypeError:
        dist.init_process_group(
            backend="nccl",
    ...
    # 第一次 barrier 显式指明本进程设备；不支持则回退裸 barrier
    try:
        dist.barrier(device_ids=[device])
    except TypeError:
        dist.barrier()

    # Python object collectives are more stable on a CPU backend than NCCL.
    ...
    if (data_args.gen_aug or data_args.gen_spatial_shift) and training_args.visual_gen:
        dataset_meta = _inject_generation_augmentation(dataset_meta, data_args, logger)
    elif data_args.gen_aug or data_args.gen_spatial_shift:
        logger.warning("Generation augmentation is enabled but visual_gen=False, so it is ignored.")
    ...
    best_mse = 0.01
    best_step = -1
    best_meta_path = os.path.join(training_args.checkpoint_dir, "best_mse.json")

    def maybe_gather_and_save(curr_step, data_status):
        """在需要保存时执行，与原保存流程一致。"""
        data_status = {} if data_status is None else _to_cpu_only(data_status)
        if dist.get_rank() == 0:
            gather_list = [None] * dist.get_world_size()
        else:
            gather_list = None
        dist.gather_object(data_status, gather_list, dst=0, group=data_status_group)
    ...
    for curr_step, data in enumerate(train_loader, start=train_step):
        ...
        if training_args.visual_gen and ('mse' in avg_metrics) and (curr_step >= 1000000):
                avg_mse = avg_metrics['mse']
                improved = avg_mse < best_mse - 1e-12  # 避免浮点噪声
                #print("avg_mse",avg_mse)
                if improved:
                    old_best = best_mse
                    best_mse = avg_mse
                    best_step = curr_step
                    #print("old_best",old_best)
                    #print("best_mse",best_mse)
                    #print("best_step",best_step)
                    #print("improved",improved)
                    
                    if dist.get_rank() == 0:
                        logger.info(f"[BEST SAVE] step={curr_step} avg_mse improved "
                                    f"from {old_best:.6f} -> {best_mse:.6f}; saving checkpoint...")
                    # 与原本保存一致：先聚合 data_status，再保存
                    maybe_gather_and_save(curr_step, data_status)
                     # 记录 best 元信息（可选）
                    if dist.get_rank() == 0:
                        try:
                            with open(best_meta_path, "w") as f:
                                json.dump({"best_mse": best_mse, "best_step": best_step}, f, indent=2)
                        except Exception as e:
                            logger.info(f"Failed to write best meta: {e}")

        # 【注意】删除了原先 “每 save_every 步固定保存” 的逻辑
        if curr_step > 0 and curr_step % 200 == 0: 
            maybe_gather_and_save(curr_step, data_status)
         #终止条件（可选）
