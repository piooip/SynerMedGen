...
class InterleavedBaseIterableDataset(DistributedIterableDataset):
    ...
    def _sample_paired_spatial_aug_params(self):
        if hasattr(self.transform, 'sample_spatial_shift'):
            return self.transform.sample_spatial_shift()
        return None

    def _add_image(self, data, image, need_loss, need_vae, need_vit, enable_cfg=True, spatial_aug_params=None):
        assert need_loss or need_vae or need_vit

        loss_image_tensor = None
        loss_num_tokens = None
        if need_loss:
            loss_image_tensor = self.transform(
                image,
                spatial_aug_params=spatial_aug_params,
                apply_augmentation=False,
            )
            height, width = loss_image_tensor.shape[1:]
            loss_num_tokens = width * height // self.transform.stride ** 2

        vae_image_tensor = None
        vae_num_tokens = None
        if need_vae:
            vae_image_tensor = self.transform(
                image,
                spatial_aug_params=spatial_aug_params,
                apply_augmentation=True,
            )
            height, width = vae_image_tensor.shape[1:]
            vae_num_tokens = width * height // self.transform.stride ** 2
        ...
        if need_loss:
            ...
            data['num_tokens'] += loss_num_tokens
            data['image_tensor_list'].append(loss_image_tensor)
        ...
        if need_vae:
            ...
            data['num_tokens'] += vae_num_tokens
            data['image_tensor_list'].append(vae_image_tensor)
        ...
        if need_vit:
            ...
            vit_image_tensor = self.vit_transform(image, spatial_aug_params=spatial_aug_params)
